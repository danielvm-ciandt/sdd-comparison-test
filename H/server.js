...
// Track online status: user_id → true (online) when they hit their endpoint
const onlineSet = new Set();

function userWithOnline(user) {
  return { ...user, online: onlineSet.has(user.id) };
}

// GET /users
app.get('/users', (req, res) => {
  const users = db.prepare('SELECT id, name FROM users').all();
  res.json(users.map(userWithOnline));
});

// GET /users/:id — also marks user as online
app.get('/users/:id', (req, res) => {
  const user = db.prepare('SELECT id, name FROM users WHERE id = ?').get(req.params.id);
  if (!user) return res.status(404).json({ error: 'not found' });
  onlineSet.add(user.id);
  res.json(userWithOnline(user));
});
...
app.get('/conversations/:id', (req, res) => {
  ...
  res.json({ ...conv, participants: participants.map(userWithOnline) });
});
